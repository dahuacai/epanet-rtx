...
void ConfigProject::loadProjectFile(const string& path) {
  ...
  _configPath = path;
 f_CompleteRecordTs=false;
 f_CompleteRecordElmt=false;
...
PointRecord::sharedPointer PointRecordFactory::createOdbcPointRecord(libconfig::Setting &setting) {
	OdbcPointRecord::sharedPointer r( new OdbcDirectPointRecord() );    
...
//    string qualCol  = syntax["QualityColumn"];
    r->setTableColumnNames(table, dateCol, tagCol, valueCol);// disenable comment for set table value dhc at 2015-1-12 
  }
  ...
  r->setName(name);//add for test scada connect dhc at 2015-1-12
  r->dbConnect(); // leaving this to application code dhc at 2015-1-12
  return r;
}
...
void ConfigProject::setGenericTimeSeriesProperties(TimeSeries::sharedPointer timeSeries, libconfig::Setting &setting) {
  ...
  if (setting.exists("pointRecord")) {
    ...
    if (_pointRecordList.find(pointRecordName) == _pointRecordList.end()) {
      ...
      timeSeries->setRecord(pointRecord);//attention dhc 2015-1-15 record 
    }
  }
...
TimeSeries::sharedPointer ConfigProject::createRuntimeStatus(Setting &setting) {
  ...
  if (setting.exists("thresholdValue")) {
    double v = getConfigDouble(setting, "thresholdValue");
    ts->setTimeThreshold(v);// modify for adapting to func decl -dhc 2015-1-14
  }
...
void ConfigProject::createSaveOptions(libconfig::Setting &saveGroup) {
  if (saveGroup.exists("staterecord")) {
    ...
    if (saveGroup.exists("save_states")) {
	  ...
	  if (saveSetting.exists("elements")){//dhc add ,if exitting specified element ,judge its parament and set records
		  Setting  &specified_Elmt=saveSetting["elements"];
		  int num_Elmt = specified_Elmt.getLength();
		  vector<string> v_elmt;
		  v_elmt.reserve(num_Elmt);
		  for (int i=0;i<num_Elmt;++i)
		  {
			  v_elmt.push_back(specified_Elmt[i]);
		  }
		  vector<Pipe::sharedPointer> pipes = _model->pipes();
		  vector<Junction::sharedPointer> junctions = _model->junctions();
		  BOOST_FOREACH(string str_Elmt,v_elmt){
			  ...
			  BOOST_FOREACH(Junction::sharedPointer  j, _model->junctions()) {		
				  if (j->name()==str_Elmt){  
					 ...
					 j->demand()->setRecord(_defaultRecord);
	  ...
	  if (!saveSetting.isList()) {
        ...
        return;
      } //enable  for add specified_elements config dhc
	  Record_num =getRecordNum(saveSetting) ;//Prepared for batch insert
      int nStates = saveSetting.getLength();
      for (int iState = 0; iState < nStates; ++iState) {
        string stateToSave = saveSetting[iState];
		//Add following for GetRecordNum ,prepared for batch process;
        if (RTX_STRINGS_ARE_EQUAL(stateToSave, "all")) {      
		      _model->setStorage(_defaultRecord);
        }
		else if (RTX_STRINGS_ARE_EQUAL(stateToSave, "junction")) {      //add pressure records dhc
			vector<Junction::sharedPointer> junctions = _model->junctions();
			BOOST_FOREACH(Junction::sharedPointer  j, _model->junctions()) {			
				j->pressure()->setRecord(_defaultRecord);
			}
     ...
     f_CompleteRecordElmt=true;
	 vector<Junction::sharedPointer> junctions = _model->junctions();
     Junction::sharedPointer j= junctions.front();
      j->head()->setRecord(_defaultRecord);//add for the last one time to go to registeridentiers() func-dhc
  } // save_states group
...
long int ConfigProject::getRecordNum(libconfig::Setting &saveSetting)
{
	long int recordname=0;
	int nStates = saveSetting.getLength();
	for (int iState = 0; iState < nStates; ++iState) {
		string stateToSave = saveSetting[iState];
		if (RTX_STRINGS_ARE_EQUAL(stateToSave, "all")) {  
			...
			long int Recordjunction=4*junctions.size();//1
			long int Recordpipe=pipes.size();//2
			long int Recordvalue=valves.size();
			long int Recordtank =4*tanks.size();//head pressure demand quality
			long int Recordpump =2*pumps.size();//energy and flow
			long int Recordreservoir =4*reservoirs.size();//head pressure demand quality
			long int RecordDMA=dmas.size();//demand
			long int RecordRelativeError=1;
		    long 	int RecordIterations=1;
			//Add Above for GetRecordNum ,prepared for batch process;
			BOOST_FOREACH(Tank::sharedPointer t, tanks) {
				int  Recordtanklevel=0;
				if (t->doesHaveHeadMeasure()) {
					Recordtanklevel+=1;			 
				}
				Recordtank+=Recordtanklevel;
			}
			recordname=Recordjunction+Recordpipe+Recordvalue+Recordtank+Recordpump+Recordreservoir+RecordDMA+RecordRelativeError+RecordIterations;
		}
		else if (RTX_STRINGS_ARE_EQUAL(stateToSave, "flow")) {
			vector<Pipe::sharedPointer> pipes = _model->pipes();
			recordname=pipes.size()+2;
		}
		else if (RTX_STRINGS_ARE_EQUAL(stateToSave, "quality")) {
			vector<Junction::sharedPointer> junctions = _model->junctions();
			recordname=junctions.size()+2;
		}
		else if (RTX_STRINGS_ARE_EQUAL(stateToSave, "measured")) {
			// save only the element states that have measured counterparts.
			vector<Junction::sharedPointer> junctions = _model->junctions();
			BOOST_FOREACH(Junction::sharedPointer j, junctions) {
				if (j->doesHaveHeadMeasure()) {
					recordname+=2;//head level
				}
				if (j->doesHaveQualityMeasure()) {
					recordname+=1;//quality
				}
			}
			vector<Pipe::sharedPointer> pipes = _model->pipes();
			BOOST_FOREACH(Pipe::sharedPointer p, pipes) {
			...
			BOOST_FOREACH(Reservoir::sharedPointer r, reservoirs) {
				if (r->doesHaveHeadMeasure()) {
					recordname+=1;//head level
				}
			}
					recordname+=2;// add relative error and iter
		} 
		else if (RTX_STRINGS_ARE_EQUAL(stateToSave, "dma_demand")) {
			vector<Dma::sharedPointer> dmas = _model->dmas();
			recordname= dmas.size()+2;
		} // dma demand
