...
/* Open input & report files */
   ...
   writelogo();//dhc writes program logo to report file.               

/* Find network size & allocate memory for data */
   writecon(FMT02);//dhc  writes string of characters to console         
   writewin(FMT100);//dhc passes character string to viewprog() in**  application which calls the EPANET DLL 
   ERRCODE(netsize());//dhc 读*.inp文件，记录节点，管段，阀门，泵等数量
   ERRCODE(allocdata());//dhc allocates memory for network data structures

/* Retrieve input data */
   ERRCODE(getdata());//dhc-reads in network data from disk file
...
*************************************************************/
   ...
   {
      n = MaxNodes + 1;
      Node = (Snode *)  calloc(n, sizeof(Snode));//给Node分配n个长度为Snode大小的连续空间， calloc()返回一个指向分配起始地址的指针
