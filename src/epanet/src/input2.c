...
/* Make pass through data file counting number of each component */
   ...
   {
   /* Skip blank lines & those beginning with a comment */ //dhc 跳过输入文件中的空白行和注释部分
      tok = strtok(line,SEPSTR);//获取开头字符串
      if (tok == NULL) continue;//dhc 如果行开头字符串是空白，跳过此次循环
      if (*tok == ';') continue;//dhc 如果行开头字符是符号;，跳过此次循环

   /* Check if line begins with a new section heading */
      if (*tok == '[')//dhc 如果开头符号是[，匹配'['后面的关键字
      {
         newsect = findmatch(tok,SectTxt);//匹配管网对象类型标号，赋值给newsect
         if (newsect >= 0)
         {
            sect = newsect;//获得新的管网对象类型标号
            if (sect == _END) break;
            continue;
         }
         else continue;
   ...
   /* Add to count of current component */
      switch(sect)
     {
       ...
       case _RESERVOIRS://dhc why not sum?
       ...
       case _CURVES:     errcode = addcurve(tok);
         break;
       case _COORDS:     errcode = addcoord(tok); //06.02.2010-woohn//dhc enable 
      break;//dhc enable 
     }
      if (errcode) break;
   }
...
/* Get pattern & curve data from temp. lists */
   ...
   if (!errcode) errcode = getcoords();//dhc dis comment 
...
*/
{
   ...
   /* Data in these sections are not used for any computations */
       case _COORDS:      /*return (0);*/ return(coordata());//dhc enable 
	 ...
	 if (findID(id,Coordlist) == NULL)
	 {
      ...
      /* Initialize list element properties */
			// c->i = MaxCoords; // bug! if coordinates are not in the same order as junctions, then this is a BAD assumption
											  //错误！如果坐标与节点排列顺序不相同，那么这是一个坏假设
			// do this later: c->i = findnode(id);
			c->i = MaxCoords;//dhc add
			strncpy(c->ID,id,MAXID);
			c->x = NULL;
			c->y = NULL;
			c->next = Coordlist;
